<template>
  <div>
    ...
    <div v-if="!initialized" class="fixed inset-0 bg-background-primary flex items-center justify-center z-50">
...
if (process.client) {
  onMounted(async () => {
    // Registrar service worker para adicionar camada extra de proteção
    ...
    document.addEventListener('change', (event) => {
      if (event.target && event.target.type === 'file' && event.target.accept && event.target.accept.includes('image')) {
    ...
    window.addEventListener('orientationchange', preventReload, { passive: false, capture: true })
    window.addEventListener('resize', preventReload, { passive: false, capture: true })
    screen.orientation?.addEventListener('change', preventReload, { passive: false, capture: true })
    
    // Listener específico para quando a orientação muda durante captura de foto
    window.addEventListener('orientationchange', () => {
      if (isCapturingPhoto) {
        console.log('Mudança de orientação durante captura de foto - salvando estado...')
    ...
    document.addEventListener('visibilitychange', (event) => {
      if (isCapturingPhoto) {
        console.log('Visibilidade mudou durante captura - prevenindo ações padrão')
        event.preventDefault()
      }
    })
    ...
    window.addEventListener('beforeunload', (event) => {
      if (isCapturingPhoto) {
        console.log('Tentativa de recarregamento prevenida durante captura de foto')
        event.preventDefault()
        event.returnValue = ''
        return ''
      }
