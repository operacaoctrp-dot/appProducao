...
async function compartilharWhatsApp() {
  ...
  try {
    ...
    const mensagemFinal = `Gerado pelo App Produção em ${agora.toLocaleString('pt-BR')}`;
...
async function baixarImagem() {
  ...
  try {
    ...
    const modalElement = document.querySelector('.fixed.inset-0.bg-black.bg-opacity-75 .bg-white');
    ...
    link.download = `dados_producao_${formatarData(itemSelecionado.value.DataFoto)}.png`;
