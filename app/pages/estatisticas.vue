    ...
    <div class="container mx-auto px-4 py-6 max-w-7xl">
      ...
      <template v-else>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <!-- Total Geral -->
          <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-2xl p-5 text-white shadow-lg shadow-blue-200 transform hover:scale-105 transition-all duration-300">
            <div class="flex items-center justify-between mb-3">
              <span class="text-3xl">📦</span>
              <span class="text-xs bg-white/20 px-2 py-1 rounded-full">Total</span>
            </div>
            <p class="text-3xl font-bold mb-1">{{ formatNumber(stats.totalGeral) }}</p>
            <p class="text-blue-100 text-sm">kg processados</p>
            <div class="mt-2 flex items-center text-xs">
              <span :class="stats.tendenciaTotal >= 0 ? 'text-green-200' : 'text-red-200'">
                {{ stats.tendenciaTotal >= 0 ? '↑' : '↓' }} {{ Math.abs(stats.tendenciaTotal).toFixed(1) }}%
              </span>
          ...
          <!-- RSS -->
          <div class="bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-2xl p-5 text-white shadow-lg shadow-emerald-200 transform hover:scale-105 transition-all duration-300">
            <div class="flex items-center justify-between mb-3">
              <span class="text-3xl">🏥</span>
              <span class="text-xs bg-white/20 px-2 py-1 rounded-full">RSS</span>
            </div>
            <p class="text-3xl font-bold mb-1">{{ formatNumber(stats.totalRss) }}</p>
            ...
            <div class="mt-2">
              ...
              <span class="text-xs text-emerald-200 mt-1">{{ stats.percentualRss.toFixed(1) }}% do total</span>
            </div>
          </div>

          <!-- GB -->
          <div class="bg-gradient-to-br from-amber-500 to-orange-500 rounded-2xl p-5 text-white shadow-lg shadow-amber-200 transform hover:scale-105 transition-all duration-300">
            <div class="flex items-center justify-between mb-3">
              <span class="text-3xl">🗑️</span>
              <span class="text-xs bg-white/20 px-2 py-1 rounded-full">GB</span>
            </div>
            <p class="text-3xl font-bold mb-1">{{ formatNumber(stats.totalGb) }}</p>
            ...
            <div class="mt-2">
              ...
              <span class="text-xs text-amber-200 mt-1">{{ stats.percentualGb.toFixed(1) }}% do total</span>
            </div>
          </div>

          <!-- RI -->
          <div class="bg-gradient-to-br from-purple-500 to-indigo-600 rounded-2xl p-5 text-white shadow-lg shadow-purple-200 transform hover:scale-105 transition-all duration-300">
            <div class="flex items-center justify-between mb-3">
              <span class="text-3xl">🏭</span>
              <span class="text-xs bg-white/20 px-2 py-1 rounded-full">RI</span>
            </div>
            <p class="text-3xl font-bold mb-1">{{ formatNumber(stats.totalRi) }}</p>
            ...
            <div class="mt-2">
              ...
              <span class="text-xs text-purple-200 mt-1">{{ stats.percentualRi.toFixed(1) }}% do total</span>
            </div>
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <div class="bg-white rounded-xl p-4 shadow-md border border-gray-100">
            <div class="flex items-center gap-3">
              ...
              <div>
                <p class="text-2xl font-bold text-gray-800">{{ stats.totalRegistros }}</p>
          ...
          <div class="bg-white rounded-xl p-4 shadow-md border border-gray-100">
            <div class="flex items-center gap-3">
              ...
              <div>
                <p class="text-2xl font-bold text-gray-800">{{ formatNumber(stats.mediaDiaria) }}</p>
          ...
          <div class="bg-white rounded-xl p-4 shadow-md border border-gray-100">
            <div class="flex items-center gap-3">
              ...
              <div>
                <p class="text-2xl font-bold text-gray-800">{{ formatNumber(stats.maiorDia) }}</p>
          ...
          <div class="bg-white rounded-xl p-4 shadow-md border border-gray-100">
            <div class="flex items-center gap-3">
              ...
              <div>
                <p class="text-2xl font-bold text-gray-800">{{ formatNumber(stats.menorDia) }}</p>
        ...
        <div class="grid lg:grid-cols-2 gap-6 mb-6">
          ...
          <div class="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
            ...
            <div class="h-72">
              <Line v-if="chartData.labels.length > 0" :data="lineChartData" :options="lineChartOptions" />
          ...
          <div class="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
            ...
            <div class="h-72">
              <Doughnut v-if="stats.totalGeral > 0" :data="doughnutChartData" :options="doughnutChartOptions" />
              <div v-else class="h-full flex items-center justify-center text-gray-400">
                Sem dados para exibir
              </div>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-lg p-6 border border-gray-100 mb-6">
          ...
          <div class="h-80">
            <Bar v-if="chartData.labels.length > 0" :data="barChartData" :options="barChartOptions" />
            <div v-else class="h-full flex items-center justify-center text-gray-400">
              Sem dados para exibir
            </div>
          </div>
        ...
        <!-- Top 5 Dias de Maior Produção -->
        <div class="bg-white rounded-2xl shadow-lg p-6 border border-gray-100 mb-6">
          <h3 class="text-lg font-semibold text-gray-800 mb-4 flex items-center gap-2">
            <span class="text-2xl">🏆</span> Top 5 - Maiores Produções
          </h3>
          <div class="space-y-3">
            <div 
            ...
            >
              ...
              <div class="flex-1">
                <p class="font-semibold text-gray-800">{{ formatDate(item.DataFoto) }}</p>
                <p class="text-xs text-gray-500">
                  RSS: {{ item.RSS }}kg | GB: {{ item.GB }}kg | RI: {{ item.RI }}kg
                </p>
              </div>
              <div class="text-right">
                <p class="text-xl font-bold text-blue-600">{{ formatNumber(item.Total) }}</p>
                <p class="text-xs text-gray-500">kg total</p>
              </div>
            </div>
            <div v-if="topProducoes.length === 0" class="text-center py-8 text-gray-400">
            ...
            <table class="w-full">
              <thead>
                <tr class="border-b border-gray-100">
                  ...
                  <th class="text-right py-3 px-4 text-sm font-semibold text-gray-600">RSS</th>
                  <th class="text-right py-3 px-4 text-sm font-semibold text-gray-600">GB</th>
                  <th class="text-right py-3 px-4 text-sm font-semibold text-gray-600">RI</th>
              ...
              <tbody>
                ...
                >
                  <td class="py-3 px-4 text-sm text-gray-800">{{ formatDate(item.DataFoto) }}</td>
                  <td class="py-3 px-4 text-sm text-right text-emerald-600 font-medium">{{ formatNumber(item.RSS) }}</td>
                  <td class="py-3 px-4 text-sm text-right text-amber-600 font-medium">{{ formatNumber(item.GB) }}</td>
                  <td class="py-3 px-4 text-sm text-right text-purple-600 font-medium">{{ formatNumber(item.RI) }}</td>
                  <td class="py-3 px-4 text-sm text-right font-bold text-blue-600">{{ formatNumber(item.Total) }}</td>
                </tr>
                <tr v-if="ultimosRegistros.length === 0">
                  <td colspan="5" class="py-8 text-center text-gray-400">Nenhum registro encontrado</td>
                </tr>
              </tbody>
            </table>
          </div>
...
const stats = computed(() => {
  ...
  const mediaP1 = primeiraMetade.length > 0 
    ? primeiraMetade.reduce((sum, item) => sum + Number(item.Total || 0), 0) / primeiraMetade.length 
    : 0
  const mediaP2 = segundaMetade.length > 0 
    ? segundaMetade.reduce((sum, item) => sum + Number(item.Total || 0), 0) / segundaMetade.length 
    : 0
  
  const tendenciaTotal = mediaP1 > 0 ? ((mediaP2 - mediaP1) / mediaP1) * 100 : 0
...
const barChartOptions = {
  ...
  plugins: {
    ...
    tooltip: {
      ...
      padding: 12,
      callbacks: {
        label: (context) => `${context.dataset.label}: ${context.parsed.y.toLocaleString('pt-BR')} kg`
      }
    }
  },
  scales: {
    ...
    tooltip: {
      ...
      callbacks: {
        label: (context) => {
          ...
          return `${context.label}: ${context.parsed.toLocaleString('pt-BR')} kg (${percentage}%)`
        }
      }
    }
  }
